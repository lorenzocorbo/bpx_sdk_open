...
VENV_DIR="${ROOT_DIR}/.venv-cibw-macos"
...
if [[ ! -x "${VENV_PYTHON}" ]]; then
    ...
    echo "Recreating .venv-cibw-macos because it does not use Python 3.11."
    create_venv=true
elif ! "${VENV_PYTHON}" -m pip --version >/dev/null 2>&1; then
    echo "Recreating .venv-cibw-macos because pip is not installed."
    create_venv=true
fi
